...
{
	function dd($arr, $exit=true, $var_dump=false){
		echo '<pre>';
		if ($var_dump)
			var_dump($arr);
		else
...
{
	function trim_content($text, $length=30){
...
{
	function category_link($category_name){
		return base_url()."category/".$category_name;
	}
}

if ( ! function_exists('image_src'))
{
	function image_src($media_type, $image_size, $image_name){
...
{
	function permalink($category_name, $news_id){
		return base_url()."details/".$category_name."/".$news_id;
	}
}
...
if( ! function_exists('createTree'))
{
	function createTree($tree) {
	    ...
	    foreach ($tree as $key => $item) {
	    	$status = $item['status']==1 ? 'active' : '' ;
	        echo "<li class='$status'>",$item['title'], "<span><a onclick='fn_edit(".$item['id'].")' href='javascript::void()'>Edit</a> | <a onclick='fn_delete(".$item['id'].")' href='javascript::void()'>Delete</a></span>", "</li>";  
...
{
	function treeOption($tree, $selected_id='', $child_prefix='') {
	    foreach ($tree as $key => $item) {
	    	$selected = $item['id']==$selected_id ? 'selected' : '' ;
	        echo '<option '.$selected.' value="'.$item['id'].'">'.$child_prefix.' '.$item['title'].'</option>';  
	        $sub_prefix = $child_prefix;
	        if (!empty($item['children'])) {
	        	$sub_prefix = $sub_prefix.'--';
	            treeOption($item['children'], $selected_id, $sub_prefix);
	        }
	    }
	}
}
